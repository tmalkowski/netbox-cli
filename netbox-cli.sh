...
if [[ ! -e "$__netbox_completion_cache" ]] || [[ ! -s "$__netbox_completion_cache" ]] || [[ $(( $( date +%s ) - $( _stat "$__netbox_completion_cache" ) )) -gt "$__netbox_cache_max_age" ]]; then
	echo "netbox-cli: updating cache for tab-completion, this should complete within 5 seconds or so. this should only be required once every $(( $__netbox_cache_max_age / 86400 )) days." >&2
